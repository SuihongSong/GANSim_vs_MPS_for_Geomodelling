...
def G_paper(
    ...
    lb_conv_channels    = 2,            # channel number of global features when concatenating with the main backbone
    wellfc_conv_channels = 4,           # channel number of well facies data when concatenating with the main backbone
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    def tofm_prob(x, res):    # obtain softmax of facies types (facies proportion channels) fm with shape of [N, channels, x_dim, y_dim]
        lod = out_sizes_log2_lg - initCv_fmap_size_log2_lg + 2 - res
        with tf.variable_scope('tofm_prob_lod%d' % lod):           
            return apply_bias(conv2d(x, fmaps=len(facies_codes), kernel=1, gain=1, use_wscale=use_wscale))
    ...
    if structure == 'linear':
        x = block(latents_in, labels_in, probimgs_in, wellfacies_in, 2)
    ...
    if structure == 'recursive':
        def grow(x, lb, prob, wellfc, res, lod):
            ...
            return img()
        imgs_out_pre = grow(latents_in, labels_in, probimgs_in, wellfacies_in, 2, out_sizes_log2_lg - initCv_fmap_size_log2_lg + 2 - 2)  
