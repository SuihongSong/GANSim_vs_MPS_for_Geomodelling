...
def conv2d(x, fmaps, kernel, gain=np.sqrt(2), padding='SAME', use_wscale=False):
    """
    CPU/GPU-safe 2D convolution.

    The network keeps tensors in NCHW format. This function internally uses NHWC,
    which works on CPU and GPU, and then converts the result back to NCHW.
    """
    w = get_weight([kernel, kernel, x.shape[1].value, fmaps], gain=gain, use_wscale=use_wscale)
    w = tf.cast(w, x.dtype)

    x = tf.transpose(x, [0, 2, 3, 1])  # NCHW -> NHWC
    x = tf.nn.conv2d(x, w, strides=[1, 1, 1, 1], padding=padding, data_format='NHWC')
...
def conv2d_mulKS(x, fmaps, kernel_x, kernel_y, gain=np.sqrt(2), padding='SAME', use_wscale=False):
    """
    CPU/GPU-safe 2D convolution with different kernel sizes along x and y.

    Input/output format is NCHW; internally uses NHWC.
    """
...
def upscale2d_conv2d(x, fmaps, kernel, gain=np.sqrt(2), use_wscale=False):
    """
    CPU/GPU-safe fused upscale + transposed convolution.
    ...
    x = tf.transpose(x, [0, 2, 3, 1])  # NCHW -> NHWC
    out_shape = [tf.shape(x)[0], x.shape[1].value * 2, x.shape[2].value * 2, fmaps]
    x = tf.nn.conv2d_transpose(x, w, out_shape, strides=[1, 2, 2, 1], padding='SAME', data_format='NHWC')
    x = tf.transpose(x, [0, 3, 1, 2])  # NHWC -> NCHW
    return x

#----------------------------------------------------------------------------
def avg_pool2d_safe(x, ksize, strides=None, padding='VALID', data_format='NCHW'):
    """
    CPU/GPU-safe average pooling.

    Input/output format is NCHW. ksize and strides can be either
    [factor_x, factor_y] or [1, 1, factor_x, factor_y].
    """
    if strides is None:
        strides = ksize

    if len(ksize) == 2:
        factor_x, factor_y = ksize
    else:
        factor_x, factor_y = ksize[2], ksize[3]

    if len(strides) == 2:
        stride_x, stride_y = strides
    else:
        stride_x, stride_y = strides[2], strides[3]

    if data_format == 'NCHW':
        x = tf.transpose(x, [0, 2, 3, 1])  # NCHW -> NHWC
        x = tf.nn.avg_pool(x, ksize=[1, factor_x, factor_y, 1], strides=[1, stride_x, stride_y, 1], padding=padding, data_format='NHWC')
        x = tf.transpose(x, [0, 3, 1, 2])  # NHWC -> NCHW
        return x
...
def downscale2d(x, factors):
    """
    CPU/GPU-safe box-filter downscaling.

    Input/output format is NCHW.
    """
    [factor_x, factor_y] = factors
    with tf.variable_scope('Downscale2d'):
        return avg_pool2d_safe(x, ksize=[factor_x, factor_y], strides=[factor_x, factor_y], padding='VALID', data_format='NCHW')

def max_pool2d_safe(x, ksize, strides=None, padding='VALID', data_format='NCHW'):
    """
    CPU/GPU-safe max pooling.
    ...
    if data_format == 'NCHW':
        x = tf.transpose(x, [0, 2, 3, 1])  # NCHW -> NHWC
        x = tf.nn.max_pool(x, ksize=[1, factor_x, factor_y, 1], strides=[1, stride_x, stride_y, 1], padding=padding, data_format='NHWC')
        x = tf.transpose(x, [0, 3, 1, 2])  # NHWC -> NCHW
        return x
...
def wellfc_downscale2d_process(x, factor_array, prior_codes): 
    ...
    facies_channel = max_pool2d_safe(facies_channel, ksize=factor_array, strides=factor_array, padding='VALID', data_format='NCHW') # shape of [N, 1, x_cells, y_cells, z_cells] e.g., [N, 1, 128, 128, 32]; non-well cells are -9
...
def conv2d_downscale2d(x, fmaps, kernel, gain=np.sqrt(2), use_wscale=False):
    """
    CPU/GPU-safe fused convolution + downscaling.
    ...
    x = tf.transpose(x, [0, 2, 3, 1])  # NCHW -> NHWC
    x = tf.nn.conv2d(x, w, strides=[1, 2, 2, 1], padding='SAME', data_format='NHWC')
    x = tf.transpose(x, [0, 3, 1, 2])  # NHWC -> NCHW
    return x
