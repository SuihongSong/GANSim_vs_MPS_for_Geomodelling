...
class Network:
    ...
    def __setstate__(self, state):
        ...
        module = types.ModuleType('_tfutil_network_import_module_%d' % len(_network_import_modules))
        ...
        if key not in self._run_cache:
            with absolute_name_scope(self.scope + '/Run'), tf.control_dependencies(None):
                if num_gpus <= 0:
                    ...
                    num_towers = num_gpus
                    devices = ['/gpu:%d' % gpu for gpu in range(num_gpus)]                
                
                in_split = list(zip(*[tf.split(x, num_towers) for x in self.input_templates]))
                out_split = []
                for tower, device in enumerate(devices):
                    with tf.device(device):
                        out_expr = self.get_output_for(*in_split[tower], return_as_list=True, **dynamic_kwargs)  
                        if out_mul != 1.0:
