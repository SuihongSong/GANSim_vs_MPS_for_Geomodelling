...
#----------------------------------------------------------------------------
desc += '_1gpu'                      # Supplement descriptions onto the folder name of results
...
G.prior_codes = [2, 1, 0]            # facies codes with decreasing priority when downsampling
...
#----------------------------------------------------------------------------
# Paths.
data_dir    = '/scratch/users/suihong/GANSim_vs_MPS_for_Geomodelling/2DChannel/0_Data/PreparedDataset/'  # Training data path
result_dir  = '/scratch/users/suihong/GANSim_vs_MPS_for_Geomodelling/2DChannel/0_Data/GANSim_cond_gf_well_prob/'  # result data path
#----------------------------------------------------------------------------
...
labeltypes                = ['orientation', 'mudproportion', 'width', 'wavelength', 'amplitude']  # can only include some labels, but must be in the same order for the label dataset; label dataset should be consistent with this 
G_loss.orit_weight        = 35   # weight for global features in the generator loss
G_loss.mudProp_weight     = 0.03
G_loss.Width_weight       = 0.4 
G_loss.wvl_weight         = 1.2 
G_loss.amplt_weight       = 5. 

if G_loss.lossnorm:       
    orit_mean = 0.721  # mean and standard deviation used to normalize losses of different global features to a Gaussian distribution.
    orit_std = 1.536
    MudProp_mean = 0.50
    MudProp_std = 0.17
    Width_mean = 4.0
...
label_size = 34 if cond_label else 0
...
G_loss.Wellfaciesloss_weight = 1500.    # Weight for well facies loss
if cond_well: desc += '-CondWell'   
if G_loss.lossnorm: G_loss.wellloss_mean = 8    # mean and standard deviation for well facies loss
if G_loss.lossnorm: G_loss.wellloss_std = 10
...
code_prob = [2, 1]   # codes of probability maps taken into the generator in the same order, should be 1 code less than facies_codes
...
if G_loss.lossnorm: G_loss.probloss_mean = 5195  # mean and standard deviation of probability loss
if G_loss.lossnorm: G_loss.probloss_std = 1177
